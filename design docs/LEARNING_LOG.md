...
## 🏦 QUESTION BANK — opened 2026-08-09 at the learner's request

**Mode change (learner, 2026-08-09):** *"bank these questions and simply finish the project. We can
still make choices, but we'll quiz on the why/learning after it's done."* Assessment is **deferred,
not cancelled** — decisions are still made by the learner, and this bank is drawn down in one
session after the build finishes. Anything taught between now and then gets **appended here**
rather than quizzed inline.

### Banked #1 — shrinkage vs `log(attempts)` (blurred 3×; the opposite-directions fixture)

Prediction. `rankTopics` computes **C from the rows** (C = 60/135 = 0.444), `m = 10`.

| topic | attempts | passes | fails | raw fail rate |
| --- | ---: | ---: | ---: | ---: |
| A `hlaalu_ledger` | 12 | 3 | 9 | 0.750 |
| B `dockside_bribe` | 120 | 72 | 48 | 0.400 |
| C `jeanus_lockbox` | 3 | 0 | 3 | 1.000 |

Ask: (a) the returned order; (b) for each position that differs from the raw-rate ordering,
**which term caused the move and why the other one didn't**.
*Answer (do not reveal):* shrunk = A .611 · C .573 · B .403; score = **B 1.931 > A 1.519 > C 0.629**.
B is LAST on both raw and shrunk rate and still ranks FIRST — the **log term** owns that. C is FIRST
on raw rate and ranks LAST — **shrinkage** owns that. This is the case the merged rule
("shrinkage guards small samples, log guards large ones") cannot produce.

### Banked #2 — derived-artefact drift, retested on the **friction rollup**

Transfer. Prod's `game_chunks` were generated by a `chunk.ts` that no longer existed; the
idempotency key could not catch it because the hash of the *new* text was never stored.
Ask: describe the concrete scenario where the **friction rollup** starts disagreeing with the raw
`events` it was derived from — a code change plus what it does — and **what an operator observes**
while it is happening.

### Banked #3 — a check that cannot pass under the failure you fear (COLD, third system)

Transfer. Not `/health`, not the query-vector cache. Pick a third system at draw-down time —
candidates: *"how would you prove the k8s CronJob actually ran last night?"* · *"how would you
prove the ingest token is being enforced?"* · *"how would you prove a new API image is the one
serving traffic?"*

### Still on the re-teach list from 07-28 (not yet re-asked)
